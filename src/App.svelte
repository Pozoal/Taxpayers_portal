<script>
  ...
  $: filteredSearchData = !!headerSearchValue
    ? searchData.filter((x) => {
        return (
          x.text.toLowerCase().includes(headerSearchValue.toLowerCase()) ||
    ...
    >
      <div class="col-md-3 col-lg-2">
        <a class="navbar-brand d-flex" href="/index.html">
          <img src="images/gra_logo.png" height="30" alt="" loading="lazy" />
          <small class="align-self-center text-secondary"
            >- TAXPAYERS PORTAL</small
          >
        </a>
      </div>
        ...
        >
          <div class="sidebar-sticky pt-3">
            <ul class="nav flex-column">
              {#each roles as role}
                <li
                  ...
                  class="nav-link py-3 border-bottom border-info"
                  on:click={(_) => activateRole(role)}
                >
                  {role}
                </li>
              {/each}
            ...
            <div class="text-muted text-center my-4">
              <small>powered by {config.company} </small>
            </div>
          </div>
        </nav> -->
        <main role="main" class="d-lg-flex col-md-10 ml-sm-auto col-lg-10">
                  ...
                  <div class="card-body p-0">
                    <div style="padding:1rem;">
                      <div class="d-flex justify-content-between">
                        ...
                        <p class="text-muted m-0">Now Playing</p>
                      </div>
                    </div>
                  </div>
                </div>
